...
public extension UIBezierPath {
 ...
 static func mx_prepare() {
  ...
  func mx_tangentAngle(atFractionOfLength t: CGFloat) -> CGFloat {
    ...
    func mx_perpendicularAngle(atFractionOfLength t: CGFloat) -> CGFloat {
      if isEmpty {
        return 0
      }
      ...
      findPathElement(at: t) { element, t in
        angle = element.perpendicularAngle(at: t)
      }
...
fileprivate struct BezierPathElement {
  ...
  func tangentAngle(at t: CGFloat) -> CGFloat {
    ...
    func perpendicularAngle(at t: CGFloat) -> CGFloat {
      switch type {
      case .addLineToPoint:
        return startPoint.linearPerpendicularAngle(to: endPoint, t: t)
      case .addQuadCurveToPoint:
        return startPoint.quadPerpendicularAngle(to: endPoint, controlPoint: controlPoints[0], t: t)
      case .addCurveToPoint:
        return startPoint.cubicPerpendicularAngle(to: endPoint, controlPoint1: controlPoints[0], controlPoint2: controlPoints[1], t: t)
      default:
        return 0
      }
    }
...
fileprivate extension CGPoint {
  ...
  func linearTangentAngle(to: CGPoint, t: CGFloat) -> CGFloat {
    ...
    func linearPerpendicularAngle(to: CGPoint, t: CGFloat) -> CGFloat {
      let dx = to.x - x;
      let dy = to.y - y;
  ...
  func quadTangentAngle(to: CGPoint, controlPoint: CGPoint, t: CGFloat) -> CGFloat {
    ...
    func quadPerpendicularAngle(to: CGPoint, controlPoint: CGPoint, t: CGFloat) -> CGFloat {
      let dx = _quadSlope(t, self.x, controlPoint.x, to.x);
      let dy = _quadSlope(t, self.y, controlPoint.y, to.y);
  ...
  func cubicTangentAngle(to: CGPoint, controlPoint1 c1: CGPoint, controlPoint2 c2: CGPoint, t: CGFloat) -> CGFloat {
    ...
    func cubicPerpendicularAngle(to: CGPoint, controlPoint1 c1: CGPoint, controlPoint2 c2: CGPoint, t: CGFloat) -> CGFloat {
      let dx = _cubicSlope(t, self.x, c1.x, c2.x, to.x);
      let dy = _cubicSlope(t, self.y, c1.y, c2.y, to.y);
